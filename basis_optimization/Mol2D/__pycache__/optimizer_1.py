...
class BasisOptimizer:
    ...
    def _callback(self, xk):
        ...
        self.alpha_history.append(xk.copy())
        energy = self.compute_energy(xk)
        self.energy_history.append(energy)

    def optimize(self, initial_alphas=None, tol=1e-4, maxiter=10, plot=True):
        ...
        res = minimize(
            ...
            options={'maxfev': maxiter, 'fatol': tol}
        )
        
        # Ensure final result is recorded
        if len(self.alpha_history) == 0 or not np.allclose(self.alpha_history[-1], res.x):
            self.alpha_history.append(res.x)
            self.energy_history.append(res.fun)
        ...
        return {
            ...
            'success': res.success,
            'iterations': res.nfev  # Actual iterations from SciPy result
        }
