...
class BasisOptimizer:
    ...
    def optimize(self, initial_alphas=None, tol=1e-4, maxiter=100, plot=True):
        ...
        # Set optimization options based on method
        options = {'maxiter': maxiter}
        if self.opt_method.lower() == 'nelder-mead':
           options['fatol'] = tol  # Tolerance on function value
           options['xatol'] = tol  # Tolerance on parameters
        elif self.opt_method.lower() in ['bfgs', 'cg', 'l-bfgs-b']:
           options['gtol'] = tol   # Gradient tolerance
        else:
           options['tol'] = tol    # General tolerance for other methods
