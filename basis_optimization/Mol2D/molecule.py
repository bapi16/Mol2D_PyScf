...
class Molecule:
    def __init__(self, geom, basis_str, charge=0, mult=1):
        self.geom_str = geom  # Store original geometry input
        self.basis_str = basis_str  # Store original basis string
        self.atoms = []
        self.basis = []
        self.units = 'bohr'
        self.charge = charge
    ...
    def parse_basis(self, basis_str):
        """Parse basis set string into structured format"""
        basis_specs = {}
        shell_map = {
            's': [[0, 0]],          
            'p': [[1, 0], [0, 1]],  
        ...
        for spec in basis_str.split(';'):
            ...
            try:
               ...
               raise ValueError(f"Invalid basis specification: '{spec}'")

            elem = elem_part.strip()
            basis_specs[elem] = []

            # Process contracted basis (with explicit coefficients)
            ...
            for match in contracted_matches:
                ...
                if len(exps) != len(coefs):
                   raise ValueError(f"Mismatched exponents/coefficients for {elem} {shell_type}")
                basis_specs[elem].append((shell_type, exps, coefs))

            # Process geometric sequence basis specifications
            remaining = re.sub(r'\w+\s+\[.*?\]\s+\[.*?\]', '', basis_part)
        
            # Split shell specs with proper parenthesis handling
            shell_specs = []
            current = []
            paren_level = 0
            for char in remaining:
                if char == '(':
                   ...
                   shell_spec = ''.join(current).strip()
                   if shell_spec:
                      shell_specs.append(shell_spec)
                   current = []
                else:
                   current.append(char)
            ...
            # Process each shell specification
            for shell_spec in shell_specs:
                shell_spec = re.sub(r'\s+', '', shell_spec)
                match = re.match(
                      r'^(\d*)([spdf])(\d*)(?:\(([\d.]+),([\d.]+)\))?$',
                      shell_spec
                )
                ...
                prefix_num, shell_type, suffix_num, alpha0_str, beta_str = match.groups()

                # Determine number of primitives
                n_primitives = int(prefix_num or suffix_num or 1)

                # Get parameters
                alpha0 = float(alpha0_str) if alpha0_str else self._get_default_exp(elem, shell_type)
                beta = float(beta_str) if beta_str else 2.0

                exponents = [alpha0 * (beta ** i) for i in range(n_primitives)]
        ...
        for atom in self.atoms:
            ...
            for shell_type, exps, coefs in basis_specs[elem]:
                for angular in shell_map.get(shell_type, []):
